            ...
            try:
                async with httpx.AsyncClient(verify=config.LLM_VERIFY_TLS) as client:
                    ...
                    if response.status_code == 200:
                        ...
                        logger.warning(f"Gemini {self.model_name} rate limited (429). Trying instant fallback to gemini-flash-latest / gemini-flash-lite-latest...")
                        for fb_model in ["gemini-flash-latest", "gemini-flash-lite-latest"]:
                            ...
                            try:
                                async with httpx.AsyncClient(verify=config.LLM_VERIFY_TLS) as fb_client:
                                    fb_resp = await fb_client.post(fb_url, headers=headers, json=payload, timeout=60.0)
                        ...
                        if attempt < max_retries - 1:
                            logger.warning(f"Gemini 429 Rate Limit. Auto-waiting {retry_delay}s for quota window reset (Attempt {attempt+1}/{max_retries})...")
                ...
                if attempt < max_retries - 1 and (rate_limited or isinstance(e, httpx.RequestError)):
                    delay = 20 if rate_limited else 3
                    logger.warning(f"Gemini request failed temporarily. Retrying in {delay} seconds... (Attempt {attempt+1}/{max_retries})")
